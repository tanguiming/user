    ...
    <body>
        <div class="page-content">
            <div class="main-container container-fluid">
                <div id="headshow">
                    <button type="button" class="btn  btn-primary btn-sm" onclick="backup();"><i class="fa fa-plus"></i>导出备份</button>
                </div>
...
function queryParams(params) {
	 ...
	 */
	function backup(){
		var actionUrl = "{{URL::route('admin.userLog.backup')}}";
		layer.confirm('备份后会清空数据库，确认？', function(rs){
			if(rs){
				$.ajax({
					type: 'get',
					url: actionUrl,
					...
					success: function(data) {
						// 验证不通过
						if (data.status != true){
							layer.confirm('备份失败！', {icon: 3}, function(index){
								layer.close(index); //关闭
							});
						}else{
						// 验证通过
							layer.confirm('备份成功！', {icon: 3}, function(index){
								layer.close(index); //关闭
							});
							tableApp.load();
						}
