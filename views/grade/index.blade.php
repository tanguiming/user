...
<html>
    ...
    <body>
        ...
        <script>
			...
			type: 2,
					skin: 'demo-class',
					title: ['等级规则', 'font-size:14px;background:#2b9af6;color:#fff'],
					move: '.layui-layer-title', //触发拖动的元素false 禁止拖拽，.layui-layer-title 可以拖拽
					area: ['760px', '380px'], //设置弹出框的宽高
					...
					content: ['{{URL::route('admin.user.usergrade.add')}}', 'no'],
					btn: ['确定', '取消']
					, yes: function (index) {

					var obj = layer.getChildFrame('#wt-forms', index); //获取form的值
							var actionUrl = "{{URL::route('admin.user.usergrade.insert')}}";
							$.ajax({
							type: 'post',
									url: actionUrl,
									data: obj.serialize(),
			...
			type: 2,
					...
					// time:1000,  设置自动关闭窗口时间 1秒=1000；
					shift: 0, //打开效果：0-6 。0放大，1从上到下，2下到上，3左到右放大，4翻滚效果；5渐变；6抖窗口
					content: ['{{URL::route('admin.user.usergrade.edit')}}?id=' + id, 'no'],
					btn: ['确定', '取消']
					, yes: function (index) {

					var obj = layer.getChildFrame('#wt-forms-edit', index); //获取form的值
							var actionUrl = "{{URL::route('admin.user.usergrade.update')}}";
							$.ajax({
							type: 'post',
									url: actionUrl,
									data: obj.serialize(),
			...
			function delmore(id) {
				...
				layer.confirm('是否确定删除？', {
					...
					var actionUrl = "{{URL::route('admin.user.usergrade.del')}}";
					$.ajax({
						type: 'get',
						url: actionUrl,
						data: 'ids=' + ids,
