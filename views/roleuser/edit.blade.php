...
<div class="modal-body">
		...
		<div class="row-fluid">
			<div class="span4">
				...
				<input type="text" required="required"  style="height:25px;" id="name" name="name" value="{{$data['name']}}" placeholder="角色名称" maxlength="50"/>
				<h5 class="lighter smaller">描述：</h3>
				<input type="text" required="required"  style="height:25px;"    id="description" name="description" value="{{$data['description']}}"   placeholder="角色描述" maxlength="100"/>
				<input type="hidden" required="required"  style="height:25px;"    id="role_id" name="" value="{{$data['role_id']}}"   placeholder="角色描述" maxlength="100"/>
				</form>
			</div>
...
<div class="modal-footer">
	<button type="button" class="btn btn-small btn-primary" onclick="updaterold();">
...
<script type="text/javascript">
	...
	jQuery(function($) {
		...
		$.ajax({
			url: "{{URL::route('admin.core.user.roleuser.roleidaca')}}",//admin.user.ajax.tree admin.aca.ajax.tree
			type: 'GET',
			dataType: 'json',
