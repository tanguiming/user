				...
				<div class="allinput" style="display:none;width:600px;margin-left:50px">
					...
					<div class="form-group">
						...
						<div class="col-xs-8">
							<select name="classid" class="form-control" id="selectchoose">
								...
								<option <?php if ($v['classid'] == $classid): ?>selected="selected"<?php endif; ?> value="{{$v['classid']}}">{{$v['classtitle']}}</option>
								@endforeach
							</select> 
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-xs-8" style="margin-top:10px;">
							<input type="text" value="{{$name}}" name="name" class="form-control input-sm" placeholder="请输入规则名称"> 
						</div>
					</div>
					<div class="form-group ">
						<label class="col-xs-3 control-label" style="margin-top:10px;">规则拼音：</label>
						<div class="col-xs-8" style="margin-top:10px;">
							<input type="text" disabled="disabled" value="{{$pinyin}}" name="pinyin" class="form-control input-sm" placeholder="请输入规则拼音"><div style="color:red;">规则拼音不可修改，请认真输入</div>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-xs-8" style="margin-top:10px;">
							<select name="chooseway" class="form-control" onchange="choicebrand(this.value)">
								...
								<option <?php if ($chooseway == 1): ?>selected="selected"<?php endif; ?> value="1">加{{$globalname}}（按固定值）</option>
								<option <?php if ($chooseway == 2): ?>selected="selected"<?php endif; ?> value="2">加{{$globalname}}（按比例）</option>
								<option <?php if ($chooseway == 3): ?>selected="selected"<?php endif; ?> value="3">减{{$globalname}}（按固定值）</option>
								<option  <?php if ($chooseway == 4): ?>selected="selected"<?php endif; ?> value="4">减{{$globalname}}（按比例）</option>
							</select>
						</div>
					</div>
					<div class="form-group " id="beans">
						@if($chooseway == 1)
						<label class="col-xs-3 control-label" style="margin-top:10px;">加{{$globalname}}（固定值）：</label>
						<div class="col-xs-8" style="margin-top:10px;">
							<input type="text" name="reward_bean" class="form-control input-sm" placeholder="请输入奖励豆" value="{{$reward_bean}}" id="trouble"><div style="color:red;">输入需要奖励的欢乐豆数，整数</div> 
						</div>
						@elseif($chooseway == 2)
						<label class="col-xs-3 control-label" style="margin-top:10px;">加{{$globalname}}（百分比）：</label>
						<div class="col-xs-8" style="margin-top:10px;">
							<input type="text" name="reward_bean" class="form-control input-sm" placeholder="请输入奖励豆" value="{{$reward_bean}}" id="trouble"> <div style="color:red;">%欢乐豆=当前欢乐豆+当前欢乐豆X 百分比</div>
						</div>
						@elseif($chooseway == 3)
						<label class="col-xs-3 control-label" style="margin-top:10px;">减{{$globalname}}（固定值）：</label>
						<div class="col-xs-8" style="margin-top:10px;">
							<input type="text" name="deduct_bean" class="form-control input-sm" placeholder="请输入奖励豆" value="{{$deduct_bean}}" id="trouble"> <div style="color:red;">输入需要奖励的欢乐豆数，整数</div>
						</div>
						@else
						<label class="col-xs-3 control-label" style="margin-top:10px;">减{{$globalname}}（百分比）：</label>
						<div class="col-xs-8" style="margin-top:10px;">
							<input type="text" name="deduct_bean" class="form-control input-sm" placeholder="请输入奖励豆" value="{{$deduct_bean}}" id="trouble"> <div style="color:red;">%欢乐豆=当前欢乐豆+当前欢乐豆X 百分比</div>
						</div>
						@endif
					</div>
					<div class="form-group "style="display:none;">
						...
						<div class="col-xs-8" style="margin-top:10px;">
							<input type="text" name="reward_bean" class="form-control input-sm" placeholder="请输入奖励豆" id="addbeans" value="{{$reward_bean}}"> 
						</div>
					</div>
					<div class="form-group "style="display:none;">
						<label class="col-xs-3 control-label" style="margin-top:10px;">扣除豆：</label>
						<div class="col-xs-8" style="margin-top:10px;">
							<input type="text" name="deduct_bean" class="form-control input-sm" placeholder="请输入扣除豆" id="jianbeans" value="{{$deduct_bean}}"> 
						</div>
					</div>
					<div class="form-group " style="display:none;">
					...
					<div class="form-group ">
						...
						<div class="col-xs-8" style="margin-top:10px;">
							<input type="text" value="{{$experience}}" name="experience" class="form-control input-sm" placeholder="请输入经验,例如:+10或-10"> 
						</div>
					</div>
					<div class="form-group ">
						<label class="col-xs-3 control-label" style="margin-top:10px;">规则描述：</label>
						<div class="col-xs-8" style="margin-top:10px;">
							<textarea name="content" rows="3" cols="43">{{$content}}</textarea> 
						</div>
					</div>
					<input type="hidden" name="enabled" class="form-control input-sm" value="{{$enabled}}"> 
					<!-- FormEnd -->
					<div class="modal-footer" style="text-align:center;">
						<button type="button" id="aniu" class="btn btn-small btn-success" onclick="window.parent.update();" style="width:180px;float:left;margin-left:180px;margin-top:20px;display:none;">
							<i class="icon-ok"></i>确认
						</button>
						<button type="button" id="anius" class="btn btn-small btn-success" onclick="updates();" style="width:180px;float:left;margin-left:180px;margin-top:20px">
							<i class="icon-ok"></i>确认
						</button>
					</div>
				</div>
...
function choicebrand(contenttypeid){
	//这里都要归0，是因为加豆和减豆只需一项有值，所以在给加减豆赋值前，先归0，在赋值
	$('#addbeans').attr('value',0);
	$('#jianbeans').attr('value',0);
	var fieldhtml = "";
...
function updates(){
	...
	console.log(reward_bean,'add');
	var deduct_bean =$("input[name^='deduct_bean']").val();
	console.log(deduct_bean,'jian');
	if(reward_bean){
